...
class FormatField:
    ...
    def __init__(self, max_length=float('inf'), id_safe=False):
        ...
        :param field_map: keyword arguments to use for test formatting
        :param id_safe: only allow fields safe for a Mollie id argument
        """
...
class PluginSettingsForm(PaymentPluginSettingsFormBase):
    ...
    apikey = StringField(
        label=_('API key'),
        validators=[DataRequired()],
        description=_('The Mollie API key')
    )
    ...
    order_description = StringField(
        ...
        description=_(
            ...
            'It is presented to the registrant during the transaction with Mollie. '
            'Event managers will be able to override this. '
...
class EventSettingsForm(PaymentEventSettingsFormBase):
    ...
    apikey = StringField(
        ...
        validators=[
            DataRequired(),
            #IndicoRegexp(r'^[-a-zA-Z0-9_]{1,50}$'),
        ],
        description=_('The Mollie API key')
    )
    # account_id = StringField(
    ...
    #         IndicoRegexp(r'^[0-9-]{0,15}$')
    #     ],
    #     description=_('The Mollie account ID, such as "123456-12345678".')
    # )
    order_description = StringField(
        label=_('Order Description'),
        validators=[DataRequired(), FormatField(max_length=80)],
