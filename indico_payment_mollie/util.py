...
# Mollie API details
MOLLIE_JSON_API_SPEC = '1.12'
MOLLIE_PP_INIT_URL = 'Payment/v1/PaymentPage/Initialize'
MOLLIE_PP_ASSERT_URL = 'Payment/v1/PaymentPage/Assert'
MOLLIE_PP_CAPTURE_URL = 'Payment/v1/Transaction/Capture'
MOLLIE_PP_CANCEL_URL = 'Payment/v1/Transaction/Cancel'
...
def validate_currency(iso_code):
    ...
    if iso_code in NON_DECIMAL_CURRENCY:
        raise HTTPNotImplemented(
            _("Unsupported currency '{}' for Mollie. Please contact the organizers").format(iso_code)
        )
    try:
        iso4217.Currency(iso_code)
    except ValueError:
        raise HTTPNotImplemented(
            _("Unknown currency '{}' for Mollie. Please contact the organizers").format(iso_code)
        )
